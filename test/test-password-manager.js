...
describe('Password manager', async function() {
    ...
    describe('security', async function() {
        ...
        it('rejects swap attacks', async function() {
            let keychain = await Keychain.init(password);
            const siteA = 'siteA.com';
            const passA = 'passA';
            const siteB = 'siteB.com';
            const passB = 'passB';

            await keychain.set(siteA, passA);
            await keychain.set(siteB, passB);
            ...
            // Manually find and swap the values in the KVS
            const hmacA = Object.keys(contentsObj.kvs).find(k => k !== 'null');
            const hmacB = Object.keys(contentsObj.kvs).find(k => k !== hmacA);
            
            const temp = contentsObj.kvs[hmacA];
            contentsObj.kvs[hmacA] = contentsObj.kvs[hmacB];
            contentsObj.kvs[hmacB] = temp;

            const tamperedContents = JSON.stringify(contentsObj);

            // We need to load without the checksum, as we tampered with the data
            let tamperedKeychain = await Keychain.load(password, tamperedContents);
            
            // Expect a rejection when trying to get the password
            await expectReject(tamperedKeychain.get(siteA));
            await expectReject(tamperedKeychain.get(siteB));
        });
